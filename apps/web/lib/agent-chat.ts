...
export async function ensureAssistantForAgent(tenantId: string, userId: string, agentId: string) {
  ...
  // Унарный ассистент для агента ещё не создан. Имя (tenantId, name) уникально,
  // поэтому сначала пробуем «усыновить» уже существующего с таким именем (если
  // пользователь создавал его вручную раньше или какой-то старый soft-deleted),
  // иначе создаём нового с уникальным именем.
  const baseName = `${agent.name} Assistant`;
  const nameTaken = await prisma.assistant.findFirst({
    where: { tenantId, name: baseName },
    include: { knowledgeLinks: { select: { knowledgeBaseId: true } } },
  });
  if (nameTaken && nameTaken.deletedAt == null) {
    const adopted = await prisma.assistant.update({
      where: { id: nameTaken.id },
      data: {
        agentId: agent.id,
        providerIntegrationId: nameTaken.providerIntegrationId ?? agent.providerIntegrationId,
        status: nameTaken.status === "ARCHIVED" ? "ACTIVE" : nameTaken.status,
      },
      include: { knowledgeLinks: { select: { knowledgeBaseId: true } } },
    });
    return { agent, assistant: adopted };
  }

  let createName = baseName;
  if (nameTaken) {
    createName = `${baseName} #${agent.id.slice(0, 6)}`;
  }
  let created;
  try {
    created = await prisma.assistant.create({
      data: {
        tenantId,
        createdById: userId,
    ...
    if (typeof e === "object" && e !== null && (e as { code?: string }).code === "P2002") {
      created = await prisma.assistant.create({
        data: {
          ...
          providerIntegrationId: agent.providerIntegrationId,
          agentId: agent.id,
          name: `${baseName} #${agent.id.slice(0, 6)}-${Date.now().toString(36)}`,
          systemPrompt: `You are assistant for agent "${agent.name}".`,
          status: "ACTIVE",
          settingsJson: { autogeneratedByAgentChat: true },
        },
      });
    } else {
      throw e;
