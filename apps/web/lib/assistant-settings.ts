...
export const ASSISTANT_TEMPLATES: AssistantTemplatePreset[] = [
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "Сначала всегда предлагай 1–3 конкретных варианта из каталога под задачу клиента (с коротким «почему подходит»), и только после этого задавай максимум один уточняющий вопрос, если он действительно нужен для следующего шага.",
      "Не устраивай анкету из нескольких вопросов подряд. Детали (бюджет, сроки, предпочтения) выясняй постепенно, а не списком в одном сообщении.",
 ...
 */
/**
 * Универсальные правила ведения диалога — применяются ко ВСЕМ ассистентам,
 * независимо от template/tone. Лечат болезнь «анкеты из 4 вопросов вместо ответа».
 * Коротко: сначала содержательное предложение/гипотеза → потом максимум одно уточнение.
 */
const DIALOGUE_STYLE_RULES = [
  "Стиль диалога:",
  "— В первом ответе сразу давай содержательное предложение, вариант или краткую гипотезу по задаче пользователя, опираясь на материалы из базы знаний. Не начинай с вопросов.",
  "— Максимум один короткий уточняющий вопрос за сообщение, и только после того, как ты уже предложил решение или направление. Не задавай подряд несколько вопросов (1., 2., 3.).",
  "— Не превращай диалог в анкету/бриф. Если пользователь сообщил задачу — предполагай разумный default и предлагай его; детали уточняй по ходу, а не все сразу.",
  "— Если запрос короткий или неполный («нужно видео для рекламы», «хочу сайт»), сформулируй 1–3 конкретных варианта/сценария под разумные допущения и в конце задай ОДИН уместный уточняющий вопрос.",
  "— Никогда не спрашивай бюджет, сроки и ещё что-то в одном сообщении. Приоритет — сначала ценность/идея, только потом одна деталь.",
  "— Если пользователь уже отвечал на вопрос, не задавай его повторно и не перефразируй — двигайся дальше и предлагай следующий шаг/решение.",
].join("\n");

export function buildPersonaDirectives(settings: AssistantPersonaSettings): string {
  const parts: string[] = [];

  parts.push(DIALOGUE_STYLE_RULES);
